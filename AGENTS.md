...
- If you add a Claude-compatible hook client, start in `PingIsland/Models/ClientProfile.swift` and wire any truly client-specific behavior from there before adding new ad-hoc switches elsewhere.
  ...
  - OMP's built-in question tool is named `ask`. The template intercepts `ask` tool calls and races a blocking Island question (Claude-style `AskUserQuestion` envelope with `expectsResponse`) against OMP's native `ui.select` dialog; the first side to produce answers wins, the loser is cancelled, and a locally-answered question is mirrored to Island as a fire-and-forget answered envelope so the pending Island question resolves. Keep the mapper's question-tool recognition scoped to `provider == .omp` for the generic `ask` name so other clients' `ask` tools are never misclassified.
