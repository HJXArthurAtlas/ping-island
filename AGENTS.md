...
- If you add a Claude-compatible hook client, start in `PingIsland/Models/ClientProfile.swift` and wire any truly client-specific behavior from there before adding new ad-hoc switches elsewhere.
  ...
  - OMP's built-in question tool is named `ask`. The template intercepts `ask` tool calls and holds the tool call for a bounded window (island-first): it sends a blocking Island question (Claude-style `AskUserQuestion` envelope with `expectsResponse` and `tool_use_id`) and only answers from the Island response; if Island is unreachable or the window elapses it falls through to the normal path so OMP's native `ask` UI runs. Do not leave a `ctx.ui` call in flight when the handler returns (cancelled dialogs can render after the tool result and never resolve); add `tool_use_id` to OMP envelopes so the app's PostToolUse cleanup can match pending questions. Keep the mapper's question-tool recognition scoped to `provider == .omp` for the generic `ask` name so other clients' `ask` tools are never misclassified.
